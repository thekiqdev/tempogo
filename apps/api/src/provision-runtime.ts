...
const url = process.env.MIGRATION_DATABASE_URL,
  password = process.env.RUNTIME_PASSWORD,
  expected = process.env.EXPECTED_DATABASE_NAME;
if (!url || !expected || !password || !/^[A-Za-z0-9_-]{32,128}$/.test(password))
  throw new Error(
    "Configure MIGRATION_DATABASE_URL, EXPECTED_DATABASE_NAME e RUNTIME_PASSWORD aleatória de 32–128 caracteres seguros",
  );
...
try {
  ...
  if ((await pool.query("SELECT 1 FROM pg_roles WHERE rolname='cronocheckpoint_runtime'")).rowCount)
    throw new Error(
      "Role já existe; preserve a credencial atual e siga o procedimento de rotação separado",
    );
  await pool.query("BEGIN");
  await pool.query(
    "CREATE ROLE cronocheckpoint_runtime LOGIN NOSUPERUSER NOCREATEDB NOCREATEROLE NOREPLICATION NOBYPASSRLS PASSWORD '" +
      password +
      "'",
  );
  ...
  console.log(
    "Runtime limitado provisionado. Configure DATABASE_URL no serviço da API e retire as credenciais de migration.",
  );
} catch {
