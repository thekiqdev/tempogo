...
const suffix = new Date().toISOString().replace(/[:.]/g, "-"),
  folder =
    (process.env.SPRINT05 === "true" ? "tmp/sprint-05/offline/" : "tmp/sprint-03/") +
...
try {
  ...
  await page.getByRole("button", { name: "Acessos", exact: true }).click();
  await page.getByLabel("Checkpoint do acesso").selectOption({ label: "Chegada" });
  await page.screenshot({ path: folder + "/before-revoke-mobile-test.png", fullPage: true });
  await expect(page.locator(".access-list")).toContainText(code);
